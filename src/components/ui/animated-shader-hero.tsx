      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
              ...
              {trustBadge.icons && (
                <div className="flex">
                  {trustBadge.icons.map((icon, index) => (
                    <span key={index} className={`text-${index === 0 ? 'cyan' : index === 1 ? 'teal' : 'sky'}-300`}>
                      {icon}
                    </span>
                  ))}
                </div>
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-4">
          ...
          <div className="space-y-2">
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold bg-gradient-to-r from-teal-300 via-cyan-400 to-sky-300 bg-clip-text text-transparent animate-fade-in-up delay-200">
              {headline.line1}
            </h1>
            <h1 className="text-5xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-cyan-300 via-teal-400 to-blue-400 bg-clip-text text-transparent animate-fade-in-up delay-400">
              {headline.line2}
            </h1>
          </div>
...
* Modified for blue/teal color scheme
*/
precision highp float;
out vec4 O;
...
void main(void) {
    ...
    for (float i=1.; i<12.; i++) {
        ...
        vec2 p=uv;
        float d=length(p);
        // Blue/teal color scheme: cyan, teal, deep blue
        col+=.00125/d*(cos(sin(i)*vec3(2,3,4))+1.);
        float b=noise(i+p+bg*1.731);
        col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
        // Teal/cyan background mixing
        col=mix(col,vec3(bg*.05,bg*.25,bg*.20),d);
    }
    O=vec4(col,1);
}`;
